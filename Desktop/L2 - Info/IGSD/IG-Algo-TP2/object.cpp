		...
		switch(draw_method)
		{
			case DRAW_WIRE :
				for (int j = 0; j < faces[i].vertex_index.size()-1; j++) {
					window.draw_line(vertices_projected[faces[i].vertex_index[j]], vertices_projected[faces[i].vertex_index[j+1]], vec3(1, 1, 1));
				}
				window.draw_line(vertices_projected[faces[i].vertex_index[0]], vertices_projected[faces[i].vertex_index[faces[i].vertex_index.size()-1]], vec3(1, 1, 1));
				break;
			case DRAW_FILL :
				// TODO => TP03 //
				break;
