...
void Window::draw_quad(vec2 p[4], vec3 c)
{
    ...
    for (int i = 0; i < 4; i++) {
        int x1 = (int)p[i].x;
        int y1 = (int)p[i].y;
        int x2 = (int)p[(i+1)%4].x;
        int y2 = (int)p[(i+1)%4].y;
        int dx = (int)abs(x2 - x1);
        int dy = (int)abs(y2 - y1);
        int xinc = (p[(i+1)%4].x>p[i].x)?1: -1;
        int yinc = (p[(i+1)%4].y>p[i].y)?1: -1;
        if(dx >dy)
        {
            int e = -dx;
            int x = (x1 <x2)?x1:x2;
            ...
            for( int i=0; i<=dx; i++)
            {
                raster_buffer_insert(x, T[y-minY]);
                e += 2*dy;
                x++;
    ...
    for (int i = 0; i < size; i++) {
        if (T[i])
        draw_horizontal_line(i + minY, T[i][0], T[i][1], c);
    }
}
